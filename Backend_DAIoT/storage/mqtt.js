...
// Add TLS certificates if configured
if (MQTT_ENV.CA_CERT) {
    options.ca = fs.readFileSync(MQTT_ENV.CA_CERT);
}
if (MQTT_ENV.CLIENT_CERT) {
    options.cert = fs.readFileSync(MQTT_ENV.CLIENT_CERT);
}
if (MQTT_ENV.CLIENT_KEY) {
    options.key = fs.readFileSync(MQTT_ENV.CLIENT_KEY);
}
...
// connected
console.log("[MQTT] Init: Connecting...");
